...
This file contains the `Utils.java` utility class used across the project for JDBC and pagination helpers. It replicates the current source from `src/main/java/com/miracle/smart_ecommerce_jpa/common/util/Utils.java` and highlights a recent change: `calculateOffset` now uses `Math.multiplyExact` to detect integer overflow and throws an explicit `IllegalArgumentException` when overflow occurs.

## Overview

- Purpose: helper methods for reading values from `ResultSet`, converting JSON arrays, and pagination utilities.
- Important change: `calculateOffset(int page, int size)` uses `Math.multiplyExact(page, size)` and catches `ArithmeticException` to throw a clear `IllegalArgumentException` on overflow. This prevents silent integer wrap-around for very large page/size values.

## Full source (copied)

```java
package com.miracle.smart_ecommerce_security.common.util;
...
public final class Utils {
    ...
    public static int calculateOffset(int page, int size) {
        try {
            ...
            // Overflow detected — surface a clear error to callers
            throw new IllegalArgumentException("Pagination offset overflow for page=" + page + " size=" + size, e);
        }
    }
...
```
We need to create a markdown file containing the Utils.java content and explanation. Already created file earlier. Now must respond to user acknowledging and maybe show path. User asked 
