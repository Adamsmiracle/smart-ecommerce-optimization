...
public class CategoryServiceImpl implements CategoryService {
     ...
     * Evicts all cache entries so new category appears in listings.
     */
    @Override
    @Transactional
    ...
    @Transactional
    @Caching(
            put  = { @CachePut(value = CATEGORIES_CACHE, key = "'id:' + #id") },
     ...
     * productCount is fetched inline — acceptable here since this is only
     * called after a write operation (create/update) where a DB round-trip
     * has already occurred. Paginated list reads use the JOIN query instead.
     */
