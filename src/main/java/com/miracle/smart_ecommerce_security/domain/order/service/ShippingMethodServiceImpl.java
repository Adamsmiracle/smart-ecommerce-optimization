...
import com.miracle.smart_ecommerce_security.domain.order.dto.ShippingMethodRequest;
import com.miracle.smart_ecommerce_security.domain.order.dto.ShippingMethodResponse;
...
public class ShippingMethodServiceImpl implements ShippingMethodService {
     ...
     * Result cached by ID; all list/active entries evicted.
     */
    @Override
    @Transactional
    @Caching(
            put  = { @CachePut(value = SHIPPING_METHODS_CACHE, key = "'id:' + #result.id") },
            evict = { @CacheEvict(value = SHIPPING_METHODS_CACHE, allEntries = true) }
    )
     ...
     * Updated entry cached by ID; all list/active entries evicted.
     */
    @Override
    @Transactional
    @Caching(
            put  = { @CachePut(value = SHIPPING_METHODS_CACHE, key = "'id:' + #id") },
            evict = { @CacheEvict(value = SHIPPING_METHODS_CACHE, allEntries = true) }
    )
     ...
     * Cached by page + size — shipping methods rarely change.
     */
    @Override
    @Transactional(readOnly = true)
    @Cacheable(value = SHIPPING_METHODS_CACHE,
