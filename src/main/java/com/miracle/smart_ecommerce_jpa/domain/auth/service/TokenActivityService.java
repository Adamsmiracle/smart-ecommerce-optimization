...
public class TokenActivityService {
    ...
    public void logTokenGeneration(String token, String userId, String role, String clientIp, String userAgent) {
        String tokenId = extractTokenId(token);
        TokenActivity activity = new TokenActivity(tokenId, userId, role, clientIp, userAgent);
        activeTokens.put(tokenId, activity);
        
        log.info("TOKEN_ACTIVITY_GENERATION - TokenId: {} - UserId: {} - Role: {} - IP: {} - UserAgent: {} - CID: {}", 
            tokenId, userId, role, clientIp, userAgent, MDC.get("correlationId"));
    }
    ...
    /**
     * Extracts token ID from full token string (last part)
     */
    private String extractTokenId(String token) {
        if (token == null || token.isBlank()) return "unknown";
        
        String[] parts = token.split(":");
        return parts.length >= 3 ? parts[2].substring(0, 8) + "..." : "invalid";
    }
